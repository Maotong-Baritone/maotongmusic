"""Bounded nine-file batch for Brahms's Op.46 and Op.47 Peters high-voice songs."""
import argparse
import hashlib
import json
import shutil
...
IDS = ('9005', '9006', '9007', '9008', '9017', '9018', '9019', '9020', '9021')
BATCH = PublicationBatch(
    ids=IDS,
    batch_id='brahms-op46-op47-high-voice-nine-20260909',
    stage_rel=Path('imports/johannes_brahms/staging/op46-op47-high-voice-singles'),
    work_titles=('4 Lieder, Op.46', '5 Lieder, Op.47'),
    log_message='新增勃拉姆斯Op.46与Op.47高声部Peters独立单曲谱9份；标题、速度、德语及高声部与钢琴编制均已核对。',
...
def apply_metadata_corrections(root=workflow.ROOT):
    """Correct only the selected Peters high-voice files and Op.47 language."""
    ...
    if set(source_by_id) != set(IDS) or set(stage_by_id) != set(IDS):
        raise ValueError('Op.46/47 correction scope changed')
    op47_ids = set(IDS[4:])
    for file_id in IDS:
        source_item = source_by_id[file_id]
        stage_item = stage_by_id[file_id]
        expected_language = '法语' if file_id in op47_ids else '德语'
    ...
    expected_titles = {
        '9006': 'No. 2 The Lied, Art Song, and Choral Texts Archive Andante, Op. 46',
        '9018': 'No. 2 Liebesgluth. Appassionato, Op. 47',
        '9020': 'No. 4 O liebliche Wangen, ihr macht mir Verlangen. Lebhaft, Op. 47',
    }
    ...
    for file_id in IDS:
        ...
        changes.append({
            ...
            'after': '高声部、钢琴',
            'evidence': 'Live IMSLP groups the selected Peters files under the high-voice edition; the rendered scores contain one vocal line with piano.',
        })
    for file_id in IDS[4:]:
        source_by_id[file_id]['language_cn'] = '德语'
        stage_by_id[file_id]['language'] = '德语'
        changes.append({
            'imslp_id': file_id, 'field': 'language', 'before': '法语',
            'after': '德语',
            'evidence': 'The live Op.47 work page identifies German and the selected Peters scores use German text.',
        })
    corrected_titles = {
        '9006': 'No. 2 Magyarisch. Andante, Op. 46',
        '9018': 'No. 2 Liebesglut. Appassionato, Op. 47',
        '9020': 'No. 4 O liebliche Wangen. Lebhaft, Op. 47',
    }
...
def record_inspection(root=workflow.ROOT):
    ...
    if set(by_id) != set(BATCH.ids):
        raise ValueError('Staged Op.46/47 scope changed before inspection record')
    ...
    inspection = {
        'label': 'Op.46与Op.47：Peters高声部独立单曲',
        'checked_on': datetime.now().astimezone().date().isoformat(),
        'recorded_at': datetime.now(timezone.utc).isoformat(),
        'proposal_only': True,
        'publication_approved': False,
        'proposed_first_publication_ids': list(BATCH.ids),
        'source_notes': '实时IMSLP作品页确认两套作品均为德语、voice/piano、Public Domain；所选九份文件属于Peters高声部组。完整谱、低声部、中声部、手稿及改编不在本批。',

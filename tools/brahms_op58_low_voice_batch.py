"""Bounded eight-file batch for Brahms Op.58 Peters low-voice songs."""
import argparse
import hashlib
import json
import shutil
...
IDS = ('305401', '305402', '305403', '305404', '305405', '305406', '305407', '305408')
...
def apply_metadata_corrections(root=workflow.ROOT):
    ...
    if set(source_by_id) != set(IDS) or set(stage_by_id) != set(IDS):
        raise ValueError('Op.58 low-voice correction scope changed')
    for file_id in IDS:
        ...
        if (source_item['category'] != '艺术歌曲' or source_item['sub_category'] != ''
            ...
            raise ValueError(f'Op.58 source metadata changed concurrently: {file_id}')
        if (stage_item['category'] != '艺术歌曲' or stage_item['sub_category'] != ''
                or stage_item['voice_types'] != '低声部'
                or stage_item['language'] != '德语'):
            raise ValueError(f'Op.58 staged metadata changed concurrently: {file_id}')
...
def record_inspection(root=workflow.ROOT):
    ...
    if set(by_id) != set(BATCH.ids):
        raise ValueError('Staged Op.58 low-voice scope changed before inspection record')
    ...
    for file_id in IDS:
        by_id[file_id].update(
            ...
            publication_note='',
            description_summary=f'来源：IMSLP #{file_id}；版本：Peters低声部版；出版：Edition Peters；编者：Max Friedlaender',
        )
    workflow.publication.atomic_bytes(manifest_path, workflow.publication.json_bytes(manifest))
    inspection = {
        'label': '8 Lieder and Songs, Op.58：Peters低声部单曲',

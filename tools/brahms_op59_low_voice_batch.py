"""Bounded eight-file batch for Brahms Op.59 Peters low-voice songs."""
import argparse
import hashlib
import json
import shutil
...
IDS = ('309180', '309181', '309182', '309183', '309184', '309185', '309186', '309187')
BATCH = PublicationBatch(
    ids=IDS,
    batch_id='brahms-op59-low-voice-eight-20260919',
    stage_rel=Path('imports/johannes_brahms/staging/op59-low-voice-singles'),
    work_titles=('8 Lieder and Songs, Op.59',),
    log_message='新增勃拉姆斯《8 Lieder and Songs, Op. 59》Peters低声部钢琴伴奏单曲8份；标题、速度、德语及编制均已核对。',
...
def apply_metadata_corrections(root=workflow.ROOT):
    ...
    if set(source_by_id) != set(IDS) or set(stage_by_id) != set(IDS):
        raise ValueError('Op.59 low-voice correction scope changed')
    for file_id in IDS:
        ...
        if (source_item['category'] != '艺术歌曲' or source_item['sub_category'] != ''
            ...
            raise ValueError(f'Op.59 source metadata changed concurrently: {file_id}')
        if (stage_item['category'] != '艺术歌曲' or stage_item['sub_category'] != ''
                or stage_item['voice_types'] != '低声部'
                or stage_item['language'] != '德语'):
            raise ValueError(f'Op.59 staged metadata changed concurrently: {file_id}')
    expected_title = 'No. 3 Regenlied. In mässiger, ruhiger bewegung, Op. 59'
    if (source_by_id['309182']['proposed_title'] != expected_title
            or stage_by_id['309182']['title'] != expected_title):
        raise ValueError('Op.59 No.3 title changed concurrently')
    ...
    corrected_title = 'No. 3 Regenlied. In mässiger, ruhiger Bewegung, Op. 59'
    source_by_id['309182']['proposed_title'] = corrected_title
    stage_by_id['309182']['title'] = corrected_title
    changes.append({
        'imslp_id': '309182', 'field': 'title', 'before': expected_title,
        'after': corrected_title,
        'evidence': 'The rendered first score page prints “In mässiger, ruhiger Bewegung.” with a capital B.',
    })
...
def record_inspection(root=workflow.ROOT):
    ...
    if set(by_id) != set(BATCH.ids):
        raise ValueError('Staged Op.59 low-voice scope changed before inspection record')
    ...
    for file_id in IDS:
        by_id[file_id].update(
            ...
            publication_note='',
            description_summary=f'来源：IMSLP #{file_id}；版本：Peters低声部版（No.1原调）；出版：Edition Peters；编者：Max Friedlaender',
        )
    workflow.publication.atomic_bytes(manifest_path, workflow.publication.json_bytes(manifest))
    inspection = {
        'label': '8 Lieder and Songs, Op.59：Peters低声部单曲',
        'checked_on': datetime.now().astimezone().date().isoformat(),
        'recorded_at': datetime.now(timezone.utc).isoformat(),
        'proposal_only': True,
        'publication_approved': False,
        'proposed_first_publication_ids': list(BATCH.ids),
        'source_notes': '实时IMSLP作品页确认Op.59为德语独唱与钢琴作品；所选八份均为Max Friedlaender编辑、Edition Peters出版并标为Public Domain的低声部版（No.1保留原调）。',

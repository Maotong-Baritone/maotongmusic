"""Bounded twelve-file batch for Brahms Op.94 and Op.95 low-voice songs."""
import argparse
import hashlib
import json
import shutil
...
OP94_IDS = ('347092', '347093', '347094', '42852', '347095')
OP95_IDS = ('347096', '43032', '347097', '43033', '347098', '43034', '43035')
IDS = OP94_IDS + OP95_IDS
TITLE_CORRECTIONS = {}
BATCH = PublicationBatch(
    ids=IDS,
    batch_id='brahms-op94-op95-low-voice-twelve-20260921',
    stage_rel=Path('imports/johannes_brahms/staging/op94-op95-low-voice-singles'),
    work_titles=('5 Lieder, Op.94', '7 Lieder, Op.95'),
    log_message='新增勃拉姆斯Op.94与Op.95低声部钢琴伴奏单曲12份；标题、速度、德语及编制均已核对。',
...
def apply_metadata_corrections(root=workflow.ROOT):
    ...
    if set(source_by_id) != set(IDS) or set(stage_by_id) != set(IDS):
        raise ValueError('Op.94/95 low-voice correction scope changed')
    for file_id in IDS:
        ...
        if file_id in TITLE_CORRECTIONS:
            ...
            if source_item['proposed_title'] != before or stage_item['title'] != before:
                raise ValueError(f'Op.94/95 title changed concurrently: {file_id}')
    ...
    for file_id in IDS:
        ...
        if file_id in OP94_IDS:
            source_by_id[file_id]['language_cn'] = '德语'
            stage_by_id[file_id]['language'] = '德语'
            changes.append({
                'imslp_id': file_id, 'field': 'language', 'before': '法语',
                'after': '德语',
                'evidence': 'The live IMSLP Op.94 work page identifies the language as German; the selected low-voice scores contain German text.',
            })
        if file_id in TITLE_CORRECTIONS:
...
def record_inspection(root=workflow.ROOT):
    ...
    if set(by_id) != set(BATCH.ids):
        raise ValueError('Staged Op.94/95 low-voice scope changed before inspection record')
    ...
    for file_id in IDS:
        by_id[file_id].update(
            ...
            publication_note='',
            description_summary=(
                f'来源：IMSLP #{file_id}；版本：低声部移调版；出版：N. Simrock（Peters再版）'
                + ('；编者：Max Friedlaender' if file_id in OP95_IDS else '')
            ),
        )
    workflow.publication.atomic_bytes(manifest_path, workflow.publication.json_bytes(manifest))
    inspection = {
        'label': '5 Lieder, Op.94与7 Lieder, Op.95：低声部单曲',
        'checked_on': datetime.now().astimezone().date().isoformat(),
        'recorded_at': datetime.now(timezone.utc).isoformat(),
        'proposal_only': True,
        'publication_approved': False,
        'proposed_first_publication_ids': list(BATCH.ids),
        'source_notes': '实时IMSLP作品页确认Op.94含五首、Op.95含七首德语独唱与钢琴作品；所选十二份均标为Public Domain的低声部版，出版为N. Simrock并由Edition Peters再版。Op.95所选版编者为Max Friedlaender。',

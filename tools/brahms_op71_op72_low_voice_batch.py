"""Bounded ten-file batch for Brahms Op.71 and Op.72 low-voice songs."""
import argparse
import hashlib
import json
import shutil
...
OP71_IDS = ('39109', '39110', '39111', '326311', '39112')
OP72_IDS = ('39137', '326313', '326314', '326315', '326316')
IDS = OP71_IDS + OP72_IDS
TITLE_CORRECTIONS = {
    '39109': (
        'No. 1 Es liebt sich so lieblich im Lenze! Anmuthig bewegt, Op. 71',
        'No. 1 Es liebt sich so lieblich im Lenze! Anmutig bewegt, Op. 71',
    ),
    '39110': (
        'No. 2 An den Mond. Nicht zu langsam und mit Anmuth, Op. 71',
        'No. 2 An den Mond. Nicht zu langsam und mit Anmut, Op. 71',
    ),
    '326311': (
        "No. 4 Willst du, dass ich geh'? Sehr lebhaft, Op. 71",
        "No. 4 Willst du, daß ich geh'? Sehr lebhaft, Op. 71",
    ),
    '326314': (
        'No. 3 O Kühler Wald. Langsam, Op. 72',
        'No. 3 O kühler Wald. Langsam, Op. 72',
    ),
}
BATCH = PublicationBatch(
    ids=IDS,
    batch_id='brahms-op71-op72-low-voice-ten-20260920',
    stage_rel=Path('imports/johannes_brahms/staging/op71-op72-low-voice-singles'),
    work_titles=('5 Songs, Op.71', '5 Songs, Op.72'),
    log_message='新增勃拉姆斯Op.71与Op.72低声部钢琴伴奏单曲10份；标题、速度、德语及编制均已核对。',
    allowed_voice_types=('低声部', '低声部、钢琴'),
...
def apply_metadata_corrections(root=workflow.ROOT):
    ...
    if set(source_by_id) != set(IDS) or set(stage_by_id) != set(IDS):
        raise ValueError('Op.71/72 low-voice correction scope changed')
    for file_id in IDS:
        source_item = source_by_id[file_id]
        stage_item = stage_by_id[file_id]
        expected_language = '法语' if file_id in OP71_IDS else '德语'
        if (source_item['category'] != '艺术歌曲' or source_item['sub_category'] != ''
            ...
            raise ValueError(f'Op.71/72 source metadata changed concurrently: {file_id}')
        if (stage_item['category'] != '艺术歌曲' or stage_item['sub_category'] != ''
            ...
            raise ValueError(f'Op.71/72 staged metadata changed concurrently: {file_id}')
        if file_id in TITLE_CORRECTIONS:
            ...
            if source_item['proposed_title'] != before or stage_item['title'] != before:
                raise ValueError(f'Op.71/72 title changed concurrently: {file_id}')
    ...
    for file_id in IDS:
        ...
        if file_id in OP71_IDS:
            source_by_id[file_id]['language_cn'] = '德语'
            stage_by_id[file_id]['language'] = '德语'
            changes.append({
                'imslp_id': file_id, 'field': 'language', 'before': '法语',
                'after': '德语',
                'evidence': 'The live IMSLP Op.71 work page identifies the language as German; the selected scores contain German text.',
            })
        if file_id in TITLE_CORRECTIONS:
...
def record_inspection(root=workflow.ROOT):
    ...
    if set(by_id) != set(BATCH.ids):
        raise ValueError('Staged Op.71/72 low-voice scope changed before inspection record')
    ...
    inspection = {
        'label': '5 Songs, Op.71与5 Songs, Op.72：低声部单曲',
        'checked_on': datetime.now().astimezone().date().isoformat(),
        'recorded_at': datetime.now(timezone.utc).isoformat(),
        'proposal_only': True,
        'publication_approved': False,
        'proposed_first_publication_ids': list(BATCH.ids),
        'source_notes': '实时IMSLP作品页确认Op.71与Op.72各含五首德语独唱与钢琴作品；所选十份均为Max Friedlaender编辑、标为Public Domain的低声部版，出版为N. Simrock并由Edition Peters再版。',

"""Bounded twelve-file batch for Brahms Op.85 and Op.86 low-voice songs."""
import argparse
import hashlib
import json
import shutil
...
OP85_IDS = ('346944', '346945', '42847', '346946', '346947', '42848')
OP86_IDS = ('42849', '42850', '42851', '346948', '346949', '346950')
IDS = OP85_IDS + OP86_IDS
TITLE_CORRECTIONS = {}
BATCH = PublicationBatch(
    ids=IDS,
    batch_id='brahms-op85-op86-low-voice-twelve-20260920',
    stage_rel=Path('imports/johannes_brahms/staging/op85-op86-low-voice-singles'),
    work_titles=('6 Lieder, Op.85', '6 Lieder, Op.86'),
    log_message='新增勃拉姆斯Op.85与Op.86低声部钢琴伴奏单曲12份；标题、速度、德语及编制均已核对。',
...
def apply_metadata_corrections(root=workflow.ROOT):
    ...
    if set(source_by_id) != set(IDS) or set(stage_by_id) != set(IDS):
        raise ValueError('Op.85/86 low-voice correction scope changed')
    for file_id in IDS:
        source_item = source_by_id[file_id]
        stage_item = stage_by_id[file_id]
        expected_language = '德语' if file_id in OP85_IDS else '英语'
        if (source_item['category'] != '艺术歌曲' or source_item['sub_category'] != ''
            ...
            raise ValueError(f'Op.85/86 source metadata changed concurrently: {file_id}')
        if (stage_item['category'] != '艺术歌曲' or stage_item['sub_category'] != ''
            ...
            raise ValueError(f'Op.85/86 staged metadata changed concurrently: {file_id}')
        if file_id in TITLE_CORRECTIONS:
            ...
            if source_item['proposed_title'] != before or stage_item['title'] != before:
                raise ValueError(f'Op.85/86 title changed concurrently: {file_id}')
    ...
    for file_id in IDS:
        ...
        if file_id in OP86_IDS:
            source_by_id[file_id]['language_cn'] = '德语'
            stage_by_id[file_id]['language'] = '德语'
            changes.append({
                'imslp_id': file_id, 'field': 'language', 'before': '英语',
                'after': '德语',
                'evidence': 'The live IMSLP Op.86 work page identifies the language as German; the selected low-voice scores contain German text.',
            })
        if file_id in TITLE_CORRECTIONS:
...
def record_inspection(root=workflow.ROOT):
    ...
    if set(by_id) != set(BATCH.ids):
        raise ValueError('Staged Op.85/86 low-voice scope changed before inspection record')
    ...
    for file_id in IDS:
        by_id[file_id].update(
            ...
            publication_note='',
            description_summary=f'来源：IMSLP #{file_id}；版本：低声部移调版；出版：N. Simrock（Peters再版）',
        )
    workflow.publication.atomic_bytes(manifest_path, workflow.publication.json_bytes(manifest))
    inspection = {
        'label': '6 Lieder, Op.85与6 Lieder, Op.86：低声部单曲',
        'checked_on': datetime.now().astimezone().date().isoformat(),
        'recorded_at': datetime.now(timezone.utc).isoformat(),
        'proposal_only': True,
        'publication_approved': False,
        'proposed_first_publication_ids': list(BATCH.ids),
        'source_notes': '实时IMSLP作品页确认Op.85与Op.86各含六首德语独唱与钢琴作品；所选十二份均标为Public Domain的低声部版，出版为N. Simrock并由Edition Peters再版。',
        'method': '保留原PDF字节；pypdf自动解析全部页面并校验SHA256；Poppler渲染全部页面；人工查看十二份全页接触表并重点核对首尾、标题、速度、语言、编制和异常页。',
        'rendering_note': '十二份普通清晰单曲的全部页面均已渲染；页数、内容连续性、可读性和末页收束正常。',

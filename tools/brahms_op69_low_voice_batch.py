"""Bounded nine-file batch for Brahms Op.69 low-voice songs."""
import argparse
import hashlib
import json
import shutil
...
IDS = ('273693', '273694', '273695', '39043', '39044', '274967', '274968', '275650', '275651')
BATCH = PublicationBatch(
    ids=IDS,
    batch_id='brahms-op69-low-voice-nine-20260919',
    stage_rel=Path('imports/johannes_brahms/staging/op69-low-voice-singles'),
    work_titles=('9 Songs, Op.69',),
    log_message='新增勃拉姆斯《9 Songs, Op. 69》低声部钢琴伴奏单曲9份；标题、速度、德语及编制均已核对。',
...
def apply_metadata_corrections(root=workflow.ROOT):
    ...
    if set(source_by_id) != set(IDS) or set(stage_by_id) != set(IDS):
        raise ValueError('Op.69 low-voice correction scope changed')
    for file_id in IDS:
        ...
        if (source_item['category'] != '艺术歌曲' or source_item['sub_category'] != ''
            ...
            raise ValueError(f'Op.69 source metadata changed concurrently: {file_id}')
        if (stage_item['category'] != '艺术歌曲' or stage_item['sub_category'] != ''
                or stage_item['voice_types'] != '低声部'
                or stage_item['language'] != '德语'):
            raise ValueError(f'Op.69 staged metadata changed concurrently: {file_id}')
    ...
    for file_id in IDS:
        ...
        changes.append({
            ...
            'after': '低声部、钢琴',
            'evidence': 'The live IMSLP page groups all nine selected files under Low voice and identifies the work instrumentation as voice and piano.',
        })
...
def record_inspection(root=workflow.ROOT):
    ...
    if set(by_id) != set(BATCH.ids):
        raise ValueError('Staged Op.69 low-voice scope changed before inspection record')
    ...
    inspection = {
        'label': '9 Songs, Op.69：低声部单曲',
        'checked_on': datetime.now().astimezone().date().isoformat(),
        'recorded_at': datetime.now(timezone.utc).isoformat(),
        'proposal_only': True,
        'publication_approved': False,
        'proposed_first_publication_ids': list(BATCH.ids),
        'source_notes': '实时IMSLP作品页确认Op.69为九首德语独唱与钢琴作品；所选九份均为Max Friedlaender编辑、标为Public Domain的低声部版，出版为N. Simrock并由Edition Peters再版；Nos.2–3为原调。',

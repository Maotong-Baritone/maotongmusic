"""Bounded eight-file batch for Brahms's 8 Lieder and Songs, Op.58."""
import hashlib
import shutil
...
IDS = ('21766', '21767', '21768', '21769', '21770', '21771', '21772', '21773')
BATCH = PublicationBatch(
    ids=IDS,
    batch_id='brahms-op58-high-voice-eight-20260908',
    stage_rel=Path('imports/johannes_brahms/staging/op58-high-voice-singles'),
    work_titles=('8 Lieder and Songs, Op.58',),
    log_message='新增勃拉姆斯《8 Lieder and Songs, Op. 58》高声部Peters独立单曲谱8份；标题、速度、德语及高声部与钢琴编制均已核对。',
...
def apply_metadata_corrections(root=workflow.ROOT):
    ...
    if set(by_id) != set(BATCH.ids):
        raise ValueError('Op.58 source scope changed')
    if any(item['voice_types'] != '声乐、钢琴' for item in by_id.values()):
        raise ValueError('Op.58 instrumentation changed concurrently')
    ...
    if set(stage_by_id) != set(BATCH.ids):
        raise ValueError('Staged Op.58 scope changed')
    if any(item['voice_types'] != '声乐、钢琴' for item in stage_by_id.values()):
        raise ValueError('Staged Op.58 instrumentation changed concurrently')
    ...
    receipt = {
        ...
        'changes': [{
            ...
            'evidence': 'Live IMSLP source identifies these eight Peters files as the edition for high voice in original keys.',
...
def record_inspection(root=workflow.ROOT):
    ...
    if set(by_id) != set(BATCH.ids):
        raise ValueError('Staged Op.58 scope changed before inspection record')
    titles = {
        '21766': 'No. 1 Blinde Kuh. Vivace',
        '21767': 'No. 2 Während des Regens. Lebhaft',
        '21768': 'No. 3 Die Spröde. Grazioso',
        '21769': 'No. 4 O komme, holde Sommernacht verschwiegen. Lebhaft und heimlich',
        '21770': 'No. 5 Schwermut. Sehr langsam',
        '21771': 'No. 6 In der Gasse. Gehend',
        '21772': 'No. 7 Vorüber. Sehr langsam',
        '21773': 'No. 8 Serenade. Grazioso',
    }
    for file_id in titles:
        by_id[file_id].update(
            ...
            publication_note='',
            description_summary=f'来源：IMSLP #{file_id}；版本：Peters高声部原调版；出版：Edition Peters No. 3691a，Plate 10277；编者：Max Friedlaender',
        )
    workflow.publication.atomic_bytes(manifest_path, workflow.publication.json_bytes(manifest))
    inspection = {
        'label': '8 Lieder and Songs, Op.58：高声部单曲',
        'checked_on': datetime.now().astimezone().date().isoformat(),
        'recorded_at': datetime.now(timezone.utc).isoformat(),
        'proposal_only': True,
        'publication_approved': False,
        'proposed_first_publication_ids': list(BATCH.ids),
        'source_notes': '实时IMSLP作品页确认Op.58、八首目录、德语、voice/piano、Public Domain，并明确#21766–#21773为高声部原调Peters单曲；完整谱、低声部扫描和手稿选集不在本批。',

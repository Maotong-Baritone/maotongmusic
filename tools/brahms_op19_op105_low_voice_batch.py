"""Bounded ten-file batch for Brahms Op.19 and Op.105 low-voice songs."""
import argparse
import hashlib
import json
import shutil
...
OP19_IDS = ('54911', '54912', '54913', '39255', '39256')
OP105_IDS = ('43319', '43320', '347100', '43321', '347101')
IDS = OP19_IDS + OP105_IDS
BATCH = PublicationBatch(
    ids=IDS,
    batch_id='brahms-op19-op105-low-voice-ten-20260922',
    stage_rel=Path('imports/johannes_brahms/staging/op19-op105-low-voice-singles'),
    work_titles=('5 Poems, Op.19', '5 Lieder, Op.105'),
    log_message='新增勃拉姆斯Op.19与Op.105低声部钢琴伴奏单曲10份；标题、速度、德语及编制均已核对。',
...
def apply_metadata_corrections(root=workflow.ROOT):
    ...
    if set(source_by_id) != set(IDS) or set(stage_by_id) != set(IDS):
        raise ValueError('Op.19/105 low-voice correction scope changed')
    for file_id in IDS:
    ...
    typo = 'No. 1 Wie Melodien zeiht es mir. Zart, Op. 105'
    if source_by_id['43319']['proposed_title'] != typo or stage_by_id['43319']['title'] != typo:
    ...
    for file_id in IDS:
        ...
        changes.append({
            ...
            'after': '低声部、钢琴',
            'evidence': 'Live IMSLP groups the selected edition as low voice; the score contains one vocal line with piano.',
        })
    for file_id in OP105_IDS:
        source_by_id[file_id]['language_cn'] = '德语'
        stage_by_id[file_id]['language'] = '德语'
        changes.append({
            'imslp_id': file_id, 'field': 'language', 'before': '法语', 'after': '德语',
            'evidence': 'The live IMSLP work page identifies German and the rendered score contains German lyrics.',
        })
    corrected = 'No. 1 Wie Melodien zieht es mir. Zart, Op. 105'
    source_by_id['43319']['proposed_title'] = corrected
    stage_by_id['43319']['title'] = corrected
    changes.append({
        'imslp_id': '43319', 'field': 'title', 'before': typo, 'after': corrected,
        'evidence': 'The live file label and first rendered score page print “Wie Melodien zieht es mir.”',
    })
...
def record_inspection(root=workflow.ROOT):
    ...
    if set(by_id) != set(IDS):
        raise ValueError('Staged Op.19/105 low-voice scope changed before inspection record')
    for file_id in IDS:
        by_id[file_id].update(
            rendered_pages=by_id[file_id]['page_count'],
            visual_check='matched_title_key_and_instrumentation', publication_note='',
            description_summary=f'来源：IMSLP #{file_id}；版本：低声部版；出版：N. Simrock（Peters再版）；编者：Max Friedlaender',
        )
    workflow.publication.atomic_bytes(manifest_path, workflow.publication.json_bytes(manifest))
    inspection = {
        'label': '5 Poems, Op.19与5 Lieder, Op.105：低声部单曲',
        ...
        'source_notes': '实时IMSLP作品页确认两套作品均为德语独唱与钢琴；所选十份均属于Public Domain低声部组，出版为N. Simrock并由Edition Peters再版，编者为Max Friedlaender。',
        'method': '保留原PDF字节；pypdf自动解析全部页面并校验SHA256；Poppler渲染全部页面；人工查看十份全页接触表并重点核对首尾、标题、速度、语言、编制和异常页。',
        'rendering_note': '十份普通清晰单曲的全部页面均已渲染；页数、内容连续性、可读性和末页收束正常。',
        ...
        'files': {
            file_id: {
                ...
                'notes': f'{by_id[file_id]["title"]}，共{by_id[file_id]["page_count"]}页；接触表及首尾页核对通过。',
                'publication_note': '',
            } for file_id in IDS
        },
    }
...
def main():
    ...
    if args.command == 'download-observed':
        ...
        print(json.dumps({'batch_id': BATCH.batch_id, 'count': len(plan['planned']),
                          ...
                          'files': [p['item']['title'] for p in plan['planned']]}, ensure_ascii=True, indent=2))

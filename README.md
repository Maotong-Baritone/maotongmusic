...
- 批量上传：登录后打开 `http://127.0.0.1:5000/batch-upload`，一次选择多份 PDF，统一填写作曲家、分类等资料，并逐份检查曲名。
...
批量上传默认每次最多 30 份 PDF、总大小不超过 500 MB，可在 `.env` 中通过 `BATCH_UPLOAD_MAX_FILES` 和 `BATCH_UPLOAD_MAX_MB` 调整。公共资料会应用到本批全部乐谱，曲名默认从文件名生成并可逐份修改；歌词可在发布后单独编辑。系统会先校验和暂存全部 PDF，再一次性发布，任何一份失败都会取消整批导入。

...
检查通过后，将生成的 `dist/` 与源文件一起提交并推送到 Git。
Cloudflare Pages 当前未配置构建命令，直接发布仓库中的 `dist/`；
发布目录由 `wrangler.toml` 固定为 `dist`。以后每次修改公开目录，
都要先运行 `python tools/build_pages.py` 并提交更新后的 `dist/`。
